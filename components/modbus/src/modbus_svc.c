...
uint8_t MODB_dBuild(ModbusRtuDataTPDF *tDataIn, unsigned char *ucDataOut)
{
	...
	else
	{
		switch(tDataIn->tFunction)
		{
			...
			case ReportSlaveID:
				ucDataOut[2] = 0x39;
				for (size_t i = 0; i < tDataIn->usDataLen; i++)
				{
					ucDataOut[3 + i] = tDataIn->ucData[i];
				}
				usCrc = dCrcCheck(ucDataOut, 2);
				ucDataOut[tDataIn->usDataLen + 4] = (usCrc >> 8) & 0x00ff;
				ucDataOut[tDataIn->usDataLen + 5] = usCrc & 0x00ff;
				return(tDataIn->usDataLen + 5);
			default:
...
void MODB_vAnalysis_Pubilc(uint8_t *ucDataIn, uint16_t usDatalen, ModbusRtuDataTPDF *tDataOut)
{
	...
	for(i = 0; i < usDatalen; i++)
	{
		...
		{
			ESP_LOGI(TAG, "MODBUS_MODE_MASTER");
		...
		{
			ESP_LOGI(TAG, "MODBUS_MODE_SLAVE");
			tDataOut->tFunction = (ModbusFunctionTPDF)ucDataIn[i + 1];
			switch(tDataOut->tFunction)
			{
				case ReadCoil:
		...
		else if(tDataOut->tModbusMstSlvMode == MODBUS_MODE_SLAVE && usDatalen == 4)
		{
			tDataOut->tFunction = (ModbusFunctionTPDF)ucDataIn[i + 1];
			switch(tDataOut->tFunction)
			{
