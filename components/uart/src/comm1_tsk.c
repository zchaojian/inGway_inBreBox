...
void vCOM1_JointDown_PassThrough_Modbus(void)
{
	...
	if(xQueueReceive(mComm1QueueSend, (void * )&tMyQueueData, (portTickType)0))
	{
		if(tMyQueueData.tMyQueueCommand.tMyQueueCommandType == Sntp)
		{
			...
			mModbusRtuDataSend->ucData[1] = 0x01;
			mModbusRtuDataSend->ucData[2] = tTimeInfo.tm_year - 100;
			mModbusRtuDataSend->ucData[3] = tTimeInfo.tm_mon + 1;
			mModbusRtuDataSend->ucData[4] = tTimeInfo.tm_mday;
			mModbusRtuDataSend->ucData[5] = tTimeInfo.tm_hour;
			mModbusRtuDataSend->ucData[6] = tTimeInfo.tm_min;
			mModbusRtuDataSend->ucData[7] = tTimeInfo.tm_sec;
			i = MODB_dBuild(mModbusRtuDataSend, (uint8_t *)mDataSend);
			URT1_dData_Send(mDataSend, i);
			esp_log_buffer_hex(COM_P, mDataSend, i);
			return;
		}
		// China mobile device discovery.
		else if(tMyQueueData.tMyQueueCommand.tMyQueueCommandType == CMDeviceDiscovery)
		{
			mModbusRtuDataSend->tFunction = 0x10;
			...
			MODB_vAnalysis_Pubilc((uint8_t *)mDataRec, j, mModbusRtuDataRec);
			if(mModbusRtuDataRec->ucEffect == MODBUS_RESULT_FAIL)
			{
				ESP_LOGE(TAG, "COM1:passtough:ana fail\n");
				return;
			}
		...
		// According to the agreement, A serial port equipment discovery.
		else if(tMyQueueData.tMyQueueCommand.tMyQueueCommandType == ReportSlaveID)
		{
			mModbusRtuDataSend->tFunction = 0x11;
			mModbusRtuDataSend->ucSlaveAddr = tMyQueueData.tMyQueueCommand.usSlaveAddr;
			i = MODB_dBuild(mModbusRtuDataSend, (uint8_t *)mDataSend);
			URT1_dData_Send(mDataSend, i);
			esp_log_buffer_hex(COM_P, mDataSend, i);
			usDelay = mPartitionTable.tCom1Config.usTimeout / 1;
			while(usDelay)
			{
				usDelay--;
				if(URT1_dRx(mDataRec, &j))
		...
		else if(tMyQueueData.tMyQueueCommand.tMyQueueCommandType == BroadcastAddr)
		{
			...
			if(mModbusRtuDataRec->ucEffect == MODBUS_RESULT_FAIL)
			{
				printf("COM1:passtough:ana fail\n");
				return;
			}
			mModbusRtuDataRec->ucEffect = MODBUS_RESULT_FAIL;
			tMyQueueData.tMyQueueCommand.tMyQueueCommandType = mModbusRtuDataRec->tFunction;
		...
		else if(tMyQueueData.tMyQueueCommand.tMyQueueCommandType == MultiDeviceCtrl)
		{
			...
			for(uint16_t n = 0; n < usCtrlDeviceNum; n++)
			{
				printf("n: %d\n",n);
		...
		if(!usDelay)
		{
			...
			{
				if (mDevice[i].tDeviceConfig.usConvertAddr == tMyQueueData.tMyQueueCommand.usSlaveAddr)
				{
					mDevice[i].tDeviceConfig.onlineState = 0;
				}	
			}
			ESP_LOGE(COM_P, "time out");
			return;
		...
		for (size_t i = 0; i < CONFIG_DEVICE_MAXSURPPORT; i++)
		{
			if (mDevice[i].tDeviceConfig.usConvertAddr == tMyQueueData.tMyQueueCommand.usSlaveAddr)
			{
				mDevice[i].tDeviceConfig.onlineState = 1;
			}
		}
		MODB_vAnalysis((uint8_t *)mDataRec, j, mModbusRtuDataRec);
		if(mModbusRtuDataRec->ucEffect == MODBUS_RESULT_FAIL)
